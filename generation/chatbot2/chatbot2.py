...
chroma_path = os.path.join(project_root, "static", "data", "chatbot2", "chroma_db")
...
try:
    ...
    logging.error(f"Error connecting to ChromaDB for chatbot2: {e}")
    import traceback
    traceback.print_exc()
    dbclient = None
    collection = None
...
if not api_key:
    logging.warning("OPENAI_API_KEY environment variable not set. Some features may not work.")
    openai_client = None
else:
    try:
        openai_client = OpenAI(api_key=api_key)
        logging.info("OpenAI client initialized for chatbot2.")
    except Exception as e:
        logging.error(f"Error initializing OpenAI client for chatbot2: {e}")
        openai_client = None
